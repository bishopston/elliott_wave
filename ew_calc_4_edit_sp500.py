...
df = pd.read_csv("/home/alexandros/tech_analysis/ew_playground/S&P 500 Historical Data_reversed_numbers.csv", sep=',',
...
if not _1_minus_2_diff:
    ...
    for i in close[(close.index(wave_2_spot)+1):]:
        ...
        #check that accessed element is max so far
        if i > max(close[(close.index(wave_2_spot)):closeindex]) and i < wave_1_spot and ((i-wave_2_spot)/global_fib_100) > 0.332:
            # access all elements after i
            for j in close[(closeindex+1):]:
                #global_max = max(close[closeindex:])
                #check that accessed element retraces at least 23.6%
                if round((i-j)/global_fib_100,4) > 0.146:
                    _1a_minus_2a_diff.append(round((i-j)/global_fib_100,4))
                    wave_2a.append(j)
                    wave_1a.append(i)

    if not _1a_minus_2a_diff:
        print("waves 1a and 2a cannot be found")
    else:
